...
class MCPCapabilityRequest(BaseModel):
    ...
    max_options: int = Field(
        default=3, description="Maximum number of server options to return"
    )
    auto_install: bool = Field(
        default=False, description="Automatically install the best match"
    )
    prefer_fastmcp: bool = Field(
        default=True, description="Prefer FastMCP compatible servers"
    )
    include_experimental: bool = Field(
        default=False, description="Include experimental/beta servers"
    )
...
class ProductionMCPTool(BaseTool):
            ...
            if all_servers_path.exists():
                with open(all_servers_path) as f:
                    ...
                    for server in servers:
                        name = server.get(
                            "name", server.get("repository_name", "unknown")
                        )
                        self._server_database[name] = server

                logger.info(
                    f"Loaded {len(self._server_database)} servers from database"
                )
            else:
        ...
        **kwargs,
        ...
        try:
            ...
            enhanced_options = self._enhance_server_options(
                matching_servers, prefer_fastmcp, include_experimental
            )[:max_options]
    ...
    def _enhance_server_options(
        self, servers: list[dict], prefer_fastmcp: bool, include_experimental: bool
        ...
        for server in servers:
            ...
            is_experimental = (
                "experimental" in server.get("tags", [])
                or "beta" in server.get("description", "").lower()
                or (db_info.get("stars", 0) or 0) < 5
            )
            ...
            option = MCPServerOption(
                ...
                repository_url=server.get(
                    "repository_url", db_info.get("repository_url", "")
                ),
    ...
    def _format_server_options(self, options: list[MCPServerOption], query: str) -> str:
        ...
        for i, option in enumerate(options, 1):
            fastmcp_indicator = (
                "🐍 FastMCP" if option.is_fastmcp_compatible else "📦 Standard"
            )
...
Or use the `install_specific_mcp_server` tool with the exact server name."""
    ...
    async def _install_server(
        self, server_option: MCPServerOption
    ...
    async def _install_with_fastmcp(
        self, server: MCPServerOption
    ) -> MCPInstallationResult:
        ...
        try:
            ...
            return MCPInstallationResult(
                ...
                available_tools=[],
                installation_notes="",
                error_message=f"FastMCP setup failed: {e!s}",
            )

    async def _install_standard_mcp(
        self, server: MCPServerOption
    ) -> MCPInstallationResult:
        ...
        try:
            ...
            return MCPInstallationResult(
                ...
                available_tools=[],
                installation_notes="",
                error_message=f"Standard installation failed: {e!s}",
            )
...
"""
    ...
    def _generate_capability_tools(self, capability: str) -> str:
        ...
        if "database" in capability_lower or "sql" in capability_lower:
            return self._database_tools_template()
        if "web" in capability_lower or "http" in capability_lower:
            return self._web_tools_template()
        if "github" in capability_lower or "git" in capability_lower:
            return self._git_tools_template()
        if "search" in capability_lower:
...
"""
        ...
        return (
            f"📦 Installed MCP Servers ({len(self.production_tool.installed_servers)}):"
            + "".join(servers_info)
        )
