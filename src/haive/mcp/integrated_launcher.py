...
def show_status():
    ...
    if fastmcp_config.exists():
        with open(fastmcp_config, 'r') as f:
            data = json.load(f)
...
def run_csv_viewer():
    ...
    subprocess.run([sys.executable, str(script_path), "--web"])
...
def install_server_interactive():
    ...
    installable = [s for s in servers if s.get('install_command') or s.get('repository_url')]
    ...
    print(f"Install: {selected_server.get('install_command', 'Manual installation required')}")
