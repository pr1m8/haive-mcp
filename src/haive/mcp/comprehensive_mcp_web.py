...
def show_dashboard():
    ...
    with col1:
        st.metric(
            "Total Servers", 
            len(df),
            help="Total number of MCP servers in database"
        )
    ...
    with col3:
        ...
        st.metric(
            "Categories", 
            categories,
            help="Number of unique server categories"
        )
...
def perform_advanced_search(query: str, methods: List[str], max_results: int):
    ...
    if results:
        tabs = st.tabs([f"📊 {method.replace('_', ' ').title()}" for method in results.keys()])
        
        for tab, (method, docs) in zip(tabs, results.items()):
            with tab:
                st.write(f"**{len(docs)} results found**")
                
                for i, doc in enumerate(docs, 1):
                    with st.expander(f"{i}. {doc.metadata.get('server_name', 'Unknown Server')}"):
                        ...
                        if method == "parent_docs" and len(doc.page_content) > 1000:
                            ...
                            if st.button(f"Show Full Content", key=f"{method}_{i}_full"):
                                st.text_area(
                                    "Full Content", 
                                    doc.page_content, 
                                    height=400,
...
def show_data_browser():
    ...
    with st.expander("🔧 Filters & Sorting", expanded=True):
        ...
        with col4:
            ...
            sort_by = st.selectbox(
                "Sort By", 
                ['stars', 'total_features', 'tools_count', 'name']
            )
    ...
    display_columns = [
        'name', 'description', 'category', 'language', 'stars', 
        'total_features', 'tools_count', 'resources_count', 'prompts_count',
        'repository_url'
    ]
    ...
    if len(event.selection.rows) > 0:
        ...
        with col2:
            ...
            st.write(f"- **Install Command:** {'Yes' if selected_server['has_install_command'] else 'No'}")
...
def show_server_details():
    ...
    server_names = [f"{s.get('name', 'Unknown')} ({s.get('category', 'unknown')})" for s in servers]
    ...
    if selected_name:
        ...
        with col2:
            ...
            st.metric("🔧 Total Features", 
                     len(server.get('tools', [])) + 
                     len(server.get('resources', [])) + 
                     len(server.get('prompts', [])))
        
        # Tabs for different information
        tabs = st.tabs(["📋 Overview", "🛠️ Tools", "📚 Resources", "💬 Prompts", "⚙️ Installation"])
        ...
        with tabs[0]:  # Overview
            ...
            with col1:
                ...
                st.write(f"- **Repository:** {server.get('repository_url', 'Not provided')}")
...
def show_analytics():
    ...
    with col3:
        ...
        st.metric("Python Servers", f"{python_servers} ({python_servers/len(df)*100:.1f}%)")
    ...
    with col2:
        ...
        category_features = df.groupby('category')['total_features'].mean().sort_values(ascending=False).head(10)
        fig = px.bar(
