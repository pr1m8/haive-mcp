...
class MCPServerInstallRequest(BaseModel):
    ...
    capability_description: str = Field(
        description="What you want the MCP server to do"
    )
    max_servers: int = Field(
        default=3, description="Maximum number of servers to consider"
    )
        ...
        **kwargs,
        ...
        try:
            ...
            matching_servers = await doc_agent.find_servers_by_capability(
                capability_description, limit=max_servers
            )
...
"""
    ...
    async def _install_server(
        self, server_info: dict, use_fastmcp: bool = True
    ...
    def _is_fastmcp_compatible(self, server_info: dict) -> bool:
        ...
        return (
            install_type in ["python", "pip"]
            or language == "python"
            or "fastmcp" in server_info.get("keywords", [])
        )

    async def _install_fastmcp_server(
        ...
        try:
            ...
            return MCPServerInstallResult(
                ...
                installation_path=server_path,
                tools_available=[],
                error_message=f"FastMCP installation failed: {e!s}",
            )

    async def _install_standard_mcp_server(
        self, server_info: dict
    ) -> MCPServerInstallResult:
        ...
        try:
            ...
            return MCPServerInstallResult(
                ...
                installation_path="",
                tools_available=[],
                error_message=f"Standard MCP installation failed: {e!s}",
            )
...
class MCPServerListTool(BaseTool):
    ...
    def _run(self, **kwargs) -> str:
        ...
        for name, info in self.dynamic_mcp_tool.installed_servers.items():
            ...
            servers_info.append(
                f"- **{name}**: {len(info.get('tools', []))} tools ({tools})"
            )

        return (
            f"Installed MCP Servers ({len(self.dynamic_mcp_tool.installed_servers)}):\n"
            + "\n".join(servers_info)
        )
