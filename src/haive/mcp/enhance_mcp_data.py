...
class GitHubDataEnhancer:
    ...
    async def fetch_github_api(self, url: str) -> Optional[Dict[str, Any]]:
    ...
    async def get_repository_info(self, repo_url: str) -> Dict[str, Any]:
        ...
        repo_data = await self.fetch_github_api(f'https://api.github.com/repos/{owner}/{repo}')
        if not repo_data:
            return {'error': 'Could not fetch repository data'}
        
        # Fetch README
        readme_data = await self.fetch_github_api(f'https://api.github.com/repos/{owner}/{repo}/readme')
        readme_content = ""
        ...
        releases = await self.fetch_github_api(f'https://api.github.com/repos/{owner}/{repo}/releases?per_page=3')
    ...
    async def get_dependencies(self, owner: str, repo: str) -> Dict[str, Any]:
        ...
        package_json = await self.fetch_github_api(f'https://api.github.com/repos/{owner}/{repo}/contents/package.json')
        ...
        pyproject = await self.fetch_github_api(f'https://api.github.com/repos/{owner}/{repo}/contents/pyproject.toml')
        ...
        requirements = await self.fetch_github_api(f'https://api.github.com/repos/{owner}/{repo}/contents/requirements.txt')
...
class MCPDataEnhancer:
    ...
    def __init__(self, github_token: Optional[str] = None):
        self.github_token = github_token
        self.data_path = Path(__file__).parent.parent.parent.parent / "data" / "mcp_servers"
    ...
    async def enhance_single_server(self, server: Dict[str, Any], enhancer: GitHubDataEnhancer) -> Dict[str, Any]:
        ...
        try:
            ...
            if github_data.get('readme_content'):
                install_instructions = self.extract_install_instructions(github_data['readme_content'])
            ...
            enhanced_server['enhancement_status'] = f"error: {str(e)}"
            return enhanced_server
    ...
    def extract_install_instructions(self, readme_content: str) -> Dict[str, str]:
        ...
        for line in lines:
            ...
            if any(keyword in line_lower for keyword in ['install', 'setup', 'getting started', 'quick start']):
                if line.startswith('#'):
    ...
    async def enhance_all_servers(self, max_servers: Optional[int] = None) -> None:
        ...
        async with GitHubDataEnhancer(self.github_token) as enhancer:
            for i, server in enumerate(servers, 1):
                logger.info(f"Processing {i}/{len(servers)}: {server.get('name', 'Unknown')}")
        ...
        output_data = {
            'metadata': {
                ...
                'enhanced_count': sum(1 for s in enhanced_servers if s.get('enhancement_status') == 'enhanced'),
                'error_count': sum(1 for s in enhanced_servers if 'error' in s.get('enhancement_status', '')),
            },
            'all_servers': enhanced_servers
        }
        
        # Save to new file
        output_path = self.data_path / f"ALL_MCP_SERVERS_ENHANCED_{int(time.time())}.json"
...
async def main():
    ...
    parser.add_argument('--max-servers', type=int, help='Maximum number of servers to process (for testing)')
