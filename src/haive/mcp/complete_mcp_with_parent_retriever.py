...
from haive.core.engine.retriever.providers.ParentDocumentRetrieverConfig import ParentDocumentRetrieverConfig
...
"""
    ...
    async def demonstrate_retrieval(self) -> None:
        ...
        parent_docs = await self.parent_retriever.aget_relevant_documents("database integration PostgreSQL")
    ...
    async def install_top_server_with_hitl(self) -> Optional[str]:
    ...
    async def _install_server(self, server_info: MCPServerInfo) -> Optional[str]:
        ...
        try:
            async with client.session(server_name) as session:
                ...
                if resources:
                    ...
                    if content.contents:
                        print(f"   Content: {content.contents[0].text if hasattr(content.contents[0], 'text') else 'Binary'}")
                ...
                if prompts:
                    ...
                    prompt_result = await session.get_prompt(
                        prompts[0].name,
                        arguments={"topic": "MCP integration patterns"}
                    )
                    if prompt_result.messages:
                        msg = prompt_result.messages[0]
                        print(f"   Generated: {msg.content.text if hasattr(msg.content, 'text') else msg}")
...
async def main():
    ...
    print(f"📊 Total servers processed: 500 (of 1,960 available)")
    print(f"🔧 Retrievers: Parent-Child + Self-Query")
    print(f"🤖 RAG Agent: BaseRAGAgent with parent retriever")
