...
class EnhancedMCPRetriever:
    ...
    async def setup_enhanced_retriever(self) -> None:
        ...
        for server_data in servers:
            ...
            language = "unknown"
            if 'python' in repo_url or '.py' in str(server_data.get('install_command', '')):
                language = "python"
            elif 'node' in repo_url or 'npm' in str(server_data.get('install_command', '')):
                language = "javascript"
...
"""
            ...
            metadata = {
                ...
                "language": language,
                "transport": server_data.get('transport_types', ['stdio'])[0] if server_data.get('transport_types') else 'stdio',
    ...
    async def demonstrate_queries(self) -> None:
        ...
        for query in queries[:3]:  # Demo first 3
            ...
            for i, doc in enumerate(docs):
                ...
                print(f"      Has Capabilities: {doc.metadata.get('has_capabilities', False)}")
                
    async def find_and_analyze_best_server(self, requirements: str) -> Optional[Document]:
        ...
        for doc in docs:
            ...
            if 'python' in requirements.lower() and doc.metadata.get('language') == 'python':
                score += 15
