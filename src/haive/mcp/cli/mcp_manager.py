...
class MCPServerManager:
    ...
    async def discover_all_sources(self, auto_install: bool = False, limit: Optional[int] = None) -> Dict:
    ...
    def _determine_template(self, server_data: Dict) -> str:
        ...
        if 'npm' in tags or 'npm' in source:
            if '@modelcontextprotocol' in server_data.get('variables', {}).get('package', ''):
                return 'npm_official'
            elif '@' in server_data.get('variables', {}).get('package', ''):
                return 'npm_scoped'
            else:
                return 'npm_community'
        elif 'git' in tags or 'github' in tags:
            return 'git_repo'
        elif 'docker' in tags:
            return 'docker_image'
        elif 'pip' in tags or 'python' in tags:
            return 'pypi_package'
        else:
            return 'npm_community'  # Default fallback
    
    async def install_servers(self, server_names: Optional[List[str]] = None,
        ...
        for server in self.downloader.servers:
            ...
            if status_filter != "all":
                if status_filter == "installed" and info['status'] != 'installed':
                    continue
                elif status_filter == "failed" and info['status'] != 'failed':
                    continue
                elif status_filter == "enabled" and not info['enabled']:
                    continue
                elif status_filter == "disabled" and info['enabled']:
                    continue
    ...
    async def health_check(self, server_names: Optional[List[str]] = None) -> Dict:
        ...
        click.echo(f"  Health rate: {healthy/total*100:.1f}%" if total > 0 else "  No servers checked")
        
        return {'results': results, 'healthy': healthy, 'total': total}
    
    async def update_servers(self, server_names: Optional[List[str]] = None, 
...
@cli.command()
@click.option('--status', type=click.Choice(['all', 'installed', 'failed', 'enabled', 'disabled']), 
              default='all', help='Filter servers by status')
...
def list_servers(ctx, status, output_format):
    ...
    if output_format == 'json':
        ...
        for server in servers:
            ...
            click.echo(f"{status_emoji} {enabled_emoji} {server['name']:<25} {server['category']:<15} {server['installation_method']}")
...
def config(ctx):
    ...
    click.echo(f"Discovery Sources: {len(manager.downloader.patterns.get('discovery_sources', []))}")
