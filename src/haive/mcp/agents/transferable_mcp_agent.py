...
class TransferableMCPAgent(MCPMixin, SimpleAgent):
    ...
    share_client: bool = Field(
        default=True, description="Whether to share MCP client with other agents"
    )
    client_pool_key: str | None = Field(
        default=None,
    ...
    async def transfer_tool_to_agent(
        self, agent: "TransferableMCPAgent", tool_name: str
    ) -> bool:
    ...
    async def transfer_all_tools_to_agent(self, agent: "TransferableMCPAgent") -> int:
    ...
    async def shared_mcp_session(
        self, session_key: str, server_name: str | None = None
    ):
    ...
    async def collaborate_with_agents(
        self, agents: list["TransferableMCPAgent"], task: str
    ) -> dict[str, Any]:
        ...
        """
        results = {"task": task, "agents": [], "shared_tools": 0, "shared_resources": 0}
        ...
        for agent in agents:
            ...
            results["agents"].append(
                {"name": agent.name, "tools_received": tools_shared}
            )
