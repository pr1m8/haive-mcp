...
class MCPDocumentationAgent(MCPMixin, DocumentAgent):
    ...
    # Resources path
    resources_path: Optional[Path] = Field(
        default=None,
        description="Path to MCP server resources"
    )
                ...
                try:
                    ...
                    if github_result and github_result.get("loaded_documents"):
                        ...
                        if latest_setup.get("installation"):
                            result["setup_instructions"] = self._generate_setup_instructions(latest_setup)
                        
                except Exception as e:
                    logger.error(f"Failed to fetch latest docs for {server_name}: {e}")
    ...
    def _create_combined_config(self, server_configs: List[MCPServerConfig]) -> MCPConfig:
        """Create combined MCP configuration.
