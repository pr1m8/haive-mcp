...
class ServerRecommendation(BaseModel):
    ...
    alternative_servers: list[str] = Field(
        default_factory=list, description="Alternative servers that could work"
    )
...
class IntelligentMCPAgent(ReactAgent):
    ...
    auto_discover: bool = Field(
        default=True, description="Automatically discover servers based on needs"
    )

    require_approval: bool = Field(
        default=True, description="Require HITL approval for installations"
    )
    ...
    capability_cache: dict[str, list[str]] = Field(
        default_factory=dict, description="Cache of capability to server mappings"
    )

    recommendation_history: list[ServerRecommendation] = Field(
        default_factory=list, description="History of recommendations"
    )

    pending_approvals: dict[str, HITLApprovalRequest] = Field(
        default_factory=dict, description="Pending approval requests"
    )
    ...
    def _create_discovery_tools(self) -> list[Any]:
        ...
        async def install_mcp_server(
            server_name: str, require_approval: bool = True
        ) -> str:
...
def create_auto_discovering_agent(
    engine: AugLLMConfig, require_approval: bool = True
) -> IntelligentMCPAgent:
...
def create_manual_discovery_agent(engine: AugLLMConfig) -> IntelligentMCPAgent:
...
async def create_agent_with_callback(
    engine: AugLLMConfig, approval_callback: Callable[[HITLApprovalRequest], bool]
) -> IntelligentMCPAgent:
