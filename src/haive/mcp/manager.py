...
class MCPRegistrationResult(BaseModel):
    ...
    connection_time: Optional[float] = Field(default=None, description="Connection time in seconds")
...
class MCPHealthStatus(BaseModel):
    ...
    error_details: Optional[str] = Field(default=None, description="Latest error details")
...
class MCPManager(BaseModel):
    ...
    auto_health_check: bool = Field(default=True, description="Enable automatic health monitoring")
    ...
    _multi_client: Optional[Any] = PrivateAttr(default=None)
    _health_check_task: Optional[asyncio.Task] = PrivateAttr(default=None)
    ...
    async def _connect_server(
        ...
        try:
            ...
            if isinstance(config, dict):
                logger.warning(f"Connect config is dict, converting to MCPServerConfig for {server_name}")
            ...
            if config.transport.value == "stdio":
                ...
                async with stdio_client(server_params) as session:
                    return await self._handle_session_connection(server_name, config, session, server_params)
    ...
    async def _handle_session_connection(self, server_name: str, config: MCPServerConfig, session, server_params):
        """Handle successful session connection and tool discovery"""
        try:
            # Store session info (simplified for now)
            self._clients[server_name] = {"session": session, "server_params": server_params}
            ...
            try:
                ...
                if hasattr(session, 'list_tools'):
                    ...
                    logger.debug(f"Session for {server_name} does not have list_tools method")
                    tool_names = []
    ...
    async def _test_server_connection(
        ...
        try:
            ...
            if isinstance(config, dict):
                logger.warning(f"Config is dict, converting to MCPServerConfig for {server_name}")
                config = MCPServerConfig(**config)
            if config.transport.value == "stdio":
                ...
                return result.returncode == 0 or "not found" not in result.stderr.decode().lower()
            elif config.transport.value == "sse":
    ...
    async def _rebuild_multi_client(self) -> None:
        ...
        if connected_clients:
            try:
                ...
                logger.debug(f"Rebuilt multi-client with {len(connected_clients)} servers")
            except Exception as e:
        ...
        return {
            "servers": {
                name: {
                    ...
                    "health": self._server_health.get(name).dict() if name in self._server_health else None
                }
                for name, status in self._server_status.items()
            },
            "summary": {
                "total_servers": len(self._servers),
                "connected_servers": len([s for s in self._server_status.values() if s == MCPServerStatus.CONNECTED]),
                "failed_servers": len([s for s in self._server_status.values() if s == MCPServerStatus.FAILED]),
