...
from self_query_mcp_agent import SelfQueryMCPAgent, EnhancedMCPDocument
...
class MCPServerInstaller:
    ...
    def __init__(self, manager_config_path: Optional[Path] = None):
        self.manager_config_path = manager_config_path or Path.home() / ".fastmcp" / "servers.json"
        self.installations: Dict[str, ServerInstallation] = {}
        
    async def detect_package_manager(self, server_data: Dict[str, Any]) -> Tuple[str, str]:
    ...
    async def add_to_fastmcp_manager(self, server_data: Dict[str, Any], pkg_manager: str):
        """Add installed server to FastMCP manager configuration"""
        
        # Load existing config
        servers = {}
        if self.manager_config_path.exists():
            with open(self.manager_config_path, 'r') as f:
                data = json.load(f)
...
class IntegratedMCPSystem:
    ...
    def get_fastmcp_servers(self) -> Dict[str, Any]:
        ...
        with open(self.installer.manager_config_path, 'r') as f:
            data = json.load(f)
...
def create_web_interface():
    ...
    st.set_page_config(
        page_title="MCP Integrated System",
        page_icon="🚀",
        layout="wide"
    )
...
def show_discovery_tab(system):
    ...
    with col2:
        search_method = st.selectbox(
            "Method",
            ["auto", "self_query", "parent_docs", "similarity"]
        )
    ...
    if search_query:
        ...
        st.write(f"**Found {len(results['documents'])} servers using {results['method']}**")
        ...
        for i, doc in enumerate(results['documents']):
            ...
            with st.expander(f"**{server_name}** - {metadata.get('category', 'unknown')}"):
...
def show_installed_tab(system):
    ...
    if not servers:
        st.info("No servers installed yet. Use the Discover tab to find and install servers.")
        return
...
def show_running_tab(system):
    ...
    for server_name, pid in list(running.items()):
        ...
        with col3:
            if st.button(f"⏹️ Stop", key=f"stop_{server_name}_{pid}"):
