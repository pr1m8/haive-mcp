...
class HaiveMCPIntegration:
    ...
    async def discover_tool(self, query: str) -> dict[str, Any] | None:
    ...
    async def create_agent_with_mcp_tool(
        self, agent_type: str = "simple", tool_query: str = "calculator"
    ) -> Any | None:
        ...
        if agent_type == "simple":
            agent = SimpleAgent(
                name=f"agent_with_{server_name}", engine=config, tools=[mcp_tool]
            )
        elif agent_type == "react":
            agent = ReactAgent(
                name=f"react_agent_with_{server_name}", engine=config, tools=[mcp_tool]
            )
        else:
...
async def demo_discovery_to_agent():
    ...
    agent = await integration.create_agent_with_mcp_tool(
        agent_type="simple", tool_query="calculator tool"
    )

    if agent:
        # Test the agent
        await agent.arun("Calculate 25 * 4")

    # Example 2: React agent with database tool

    react_agent = await integration.create_agent_with_mcp_tool(
        agent_type="react", tool_query="database query tool"
    )

    if react_agent:
        # Test the agent
        await react_agent.arun("Query the database for user information")
...
async def demo_manual_integration():
    ...
    results = await integration.mcp_system.search_servers(
        "Python web scraping tools", method="self_query"
    )

    if results["documents"]:
        for _i, _doc in enumerate(results["documents"][:3], 1):
            pass
