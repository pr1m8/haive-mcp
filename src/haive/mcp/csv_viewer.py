...
def load_mcp_servers_data() -> List[Dict[str, Any]]:
    ...
    data_path = Path(__file__).parent.parent.parent.parent / "data" / "mcp_servers" / "ALL_MCP_SERVERS_COMPLETE.json"
...
def create_csv_export() -> pd.DataFrame:
    ...
    for server in servers:
        row = {
            ...
            'installation_notes': server.get('installation_notes', 'Standard installation'),
            ...
            'total_features': len(server.get('tools', [])) + len(server.get('resources', [])) + len(server.get('prompts', []))
        }
...
def export_to_csv():
    ...
    output_path = Path(__file__).parent.parent.parent.parent / "data" / "mcp_servers" / f"mcp_servers_export_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
...
def streamlit_viewer():
    ...
    st.set_page_config(
        page_title="MCP Servers Browser",
        page_icon="🔍",
        layout="wide"
    )
    ...
    # Features filter
    min_features = st.sidebar.number_input("Minimum Total Features", min_value=0, value=0)
    ...
    sort_columns = ['stars', 'total_features', 'tools_count', 'resources_count', 'prompts_count', 'name']
    ...
    display_columns = [
        ...
        'repository_url', 'has_install_command'
    ]
    ...
    if len(filtered_df) > 0:
        ...
        top_starred = filtered_df.nlargest(10, 'stars')[['name', 'stars', 'category', 'total_features']]
...
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--csv':
        ...
        subprocess.run([
            'streamlit', 'run', str(script_path), '--', '--streamlit'
        ])
