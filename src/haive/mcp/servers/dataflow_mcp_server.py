...
async def create_agent(request: AgentCreationRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        aug_config = AugLLMConfig(
            ...
            name=request.name,
            tools=request.tools,
            system_message=request.system_prompt or f"You are {request.name}, a helpful AI assistant."
        )
...
async def get_registry_statistics() -> Dict[str, Any]:
    ...
    try:
        ...
        stats["recent_additions"] = [
            entity.name for entity in entities[:5]
        ]
...
@mcp.prompt()
async def component_search_prompt(
    requirement: str
) -> List[Dict[str, str]]:
