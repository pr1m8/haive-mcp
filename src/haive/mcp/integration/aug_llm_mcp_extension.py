...
class MCPAugLLMConfig(AugLLMConfig):
    ...
    mcp_resources: list[MCPResource] | None = Field(
        None, description="MCP resources available to the agent"
    )

    mcp_prompts: dict[str, MCPPromptTemplate] | None = Field(
        None, description="MCP prompt templates"
    )

    mcp_manager: MCPManager | None = Field(
        None, description="MCP manager instance (not serialized)", exclude=True
    )

    # Control flags
    auto_discover_mcp_tools: bool = Field(
        True, description="Automatically discover and add MCP tools"
    )

    inject_mcp_resources: bool = Field(
        True, description="Inject MCP resources into agent context"
    )

    use_mcp_prompts: bool = Field(
        True, description="Use MCP prompts to enhance system prompts"
    )
...
def extend_aug_llm_config_for_mcp(
    base_config: AugLLMConfig, mcp_config: MCPConfig
) -> MCPAugLLMConfig:
