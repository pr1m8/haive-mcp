...
from haive.core.engine.embedding import HuggingFaceEmbeddingConfig
...
{category} {language} MCP server {name.lower().replace('-', ' ')} database python nodejs javascript typescript sql postgresql mysql sqlite github file system web api tools resources prompts
        ...
        print(f"✅ Created {len(parent_docs)} parent documents and {len(child_docs)} child chunks")
...
class SelfQueryMCPAgent:
    ...
    async def search_with_self_query(self, query: str, k: int = 5) -> List[Document]:
        ...
        try:
            docs = await asyncio.to_thread(
                self.self_query_retriever.get_relevant_documents,
                query
            )
    ...
    async def search_with_parent_retriever(self, query: str, k: int = 5) -> List[Document]:
        """Search using parent document retriever for full content."""
        print(f"🔍 Parent document search: {query}")
        
        try:
            docs = await asyncio.to_thread(
                self.parent_retriever.get_relevant_documents,
                query
            )
    ...
    def analyze_query_intent(self, query: str) -> str:
        ...
        structured_indicators = [
            "with", "having", "stars", "category", "language", 
            "tools", "resources", "prompts", "features"
        ]
        
        # Content query indicators  
        content_indicators = [
            "how to", "install", "setup", "configure", "example",
            "documentation", "readme", "instructions"
        ]
        
        if any(indicator in query_lower for indicator in structured_indicators):
            return "self_query"
        elif any(indicator in query_lower for indicator in content_indicators):
            return "parent_docs"
        else:
            return "similarity"
