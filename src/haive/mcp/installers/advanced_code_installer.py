...
from .safe_pattern_installer import SafePatternInstaller, InstallationResult
...
class CodeGenerationRequest(BaseModel):
    ...
    context_documents: List[str] = Field(default_factory=list, description="Context from documentation")
...
class GeneratedInstallPlan(BaseModel):
    ...
    environment_setup: Dict[str, str] = Field(default_factory=dict, description="Environment variables needed")
    ...
    fallback_to_safe: bool = Field(default=False, description="Whether to fallback to safe patterns")
...
class AdvancedCodeInstaller:
    ...
    def __init__(self, config_manager: Optional[MCPConfigManager] = None):
        ...
        print(f"🧠 Advanced Code Installer initialized")
        print(f"⚠️  Human oversight required for all generated code")
    ...
    def _mock_llm_code_generation(self, request: CodeGenerationRequest) -> GeneratedInstallPlan:
        ...
        elif 'python' in request.server_description.lower():
            # Python-based server
            return GeneratedInstallPlan(
        ...
        elif 'git' in package_name.lower() or 'github' in request.package_info.get('repo', ''):
    ...
    async def generate_installation_plan(self, request: CodeGenerationRequest) -> GeneratedInstallPlan:
    ...
    def create_subprocess_execution_tool(self, plan: GeneratedInstallPlan, request: CodeGenerationRequest) -> StructuredTool:
        """Create tool for executing subprocess commands with oversight"""
        
        def execute_subprocess(command: str, timeout: int = 30, working_directory: Optional[str] = None, environment_vars: Dict[str, str] = None) -> str:
    ...
    def create_validation_tool(self, plan: GeneratedInstallPlan, request: CodeGenerationRequest) -> StructuredTool:
                ...
                if "npx" in step.lower():
                    try:
                        ...
                        if result.returncode == 0:
                            ...
                            results.append(f"   ❌ npx not available")
                    except:
                        results.append(f"   ❌ npx check failed")
                    ...
                    try:
                        ...
                        if result.returncode == 0:
                            ...
                            results.append(f"   ❌ pip not available")
                    except:
                        results.append(f"   ❌ pip check failed")
                    ...
                    try:
                        ...
                        if result.returncode == 0:
                            ...
                            results.append(f"   ❌ git not available")
                    except:
                        results.append(f"   ❌ git check failed")
                
                else:
                    results.append(f"   ⏭️  Manual validation required")
            
            return "\n".join(results)
        
        return validate_installation
    
    async def install_server_advanced(self, request: CodeGenerationRequest) -> Tuple[bool, str, List[StructuredTool]]:
        ...
        if plan.fallback_to_safe and plan.confidence_score > 0.7:
            print(f"🛡️  High confidence + safe fallback - using SafePatternInstaller")
            
            # Try to find matching pattern
            from .safe_pattern_installer import InstallationRequest as SafeRequest
        ...
        def create_server_config() -> str:
            ...
            try:
                ...
                if success:
                    ...
                    return f"❌ Failed to create configuration"
