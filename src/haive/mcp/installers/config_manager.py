...
class MCPEnvironmentConfig(BaseModel):
    ...
    secure_vars: Dict[str, SecretStr] = Field(default_factory=dict, description="Secure environment variables")
    
    # Server configuration
    startup_args: List[str] = Field(default_factory=list, description="Command line arguments")
    working_directory: Optional[str] = Field(default=None, description="Working directory for server")
    
    # Transport settings
    transport_type: str = Field(default="stdio", description="stdio, http, websocket")
    port: Optional[int] = Field(default=None, description="Port for HTTP/WebSocket transports")
    
    # Security settings
    allowed_paths: List[str] = Field(default_factory=list, description="Allowed filesystem paths")
    requires_approval: bool = Field(default=True, description="Require human approval for installation")
...
class MCPConfigManager:
    ...
    def get_pattern(self, pattern_name: str) -> Optional[MCPServerPattern]:
        """Get a server pattern by name"""
        if not self.patterns_config.exists():
            return None
            
        with open(self.patterns_config, 'r') as f:
    ...
    def _update_env_file(self, config: MCPEnvironmentConfig):
        ...
        for key, secret_value in config.secure_vars.items():
            secret_str = secret_value.get_secret_value() if hasattr(secret_value, 'get_secret_value') else str(secret_value)
    ...
    def get_server_config(self, server_name: str) -> Optional[MCPEnvironmentConfig]:
    ...
    def export_claude_desktop_config(self, server_name: str) -> Optional[Dict[str, Any]]:
        ...
        claude_config = {
            ...
            "args": ["-y", config.package_name] + config.startup_args if config.package_name.startswith("@") else ["-m", config.package_name.replace("-", "_")] + config.startup_args,
        }
        
        # Add environment variables if any
        all_env = {**config.env_vars}
        for key, secret_val in config.secure_vars.items():
            all_env[key] = secret_val.get_secret_value() if hasattr(secret_val, 'get_secret_value') else str(secret_val)
