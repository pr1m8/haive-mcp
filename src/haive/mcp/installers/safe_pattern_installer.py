...
from .config_manager import MCPConfigManager, MCPEnvironmentConfig, MCPServerPattern


class InstallationRequest(BaseModel):
    """Request for MCP server installation"""
    server_name: str = Field(description="Unique name for this server instance")
    package_name: str = Field(description="Package name (e.g., @modelcontextprotocol/server-filesystem)")
    pattern_type: str = Field(description="Pattern type: filesystem, database, web_api, python_server")
...
class SafePatternInstaller:
    ...
    def __init__(self, config_manager: Optional[MCPConfigManager] = None):
        ...
        print(f"🛡️  Safe Pattern Installer initialized")
        print(f"📁 Config directory: {self.config_manager.config_dir}")
        print(f"🔧 Available patterns: {', '.join(self.config_manager.list_available_patterns())}")
    
    def get_pattern_for_server(self, package_name: str, suggested_pattern: str) -> Optional[MCPServerPattern]:
        ...
        elif "database" in package_name.lower() or "sql" in package_name.lower():
            return self.config_manager.get_pattern("database")
        elif "@modelcontextprotocol" in package_name:
    ...
    def validate_installation_request(self, request: InstallationRequest) -> Tuple[bool, str]:
        ...
        if pattern.security_level == "high_risk":
            return False, f"Pattern {request.pattern_type} is marked as high risk and not allowed in safe mode"
        
        # Validate package name format
        if not self._is_safe_package_name(request.package_name):
            return False, f"Package name '{request.package_name}' does not match safe patterns"
    ...
    def create_installation_tool(self, request: InstallationRequest) -> StructuredTool:
        ...
        def install_server_func(confirm: bool = True) -> str:
            ...
            try:
                ...
                if not config_saved:
                    return f"❌ Failed to save server configuration"
        ...
        try:
            ...
            if config.transport_type == "stdio":
                startup_cmd = pattern.startup_command.format(
                    ...
                    args=" ".join(config.startup_args)
                ).split()
                
                # Prepare environment
                env = {**config.env_vars}
                for key, secret_val in config.secure_vars.items():
                    env[key] = secret_val.get_secret_value() if hasattr(secret_val, 'get_secret_value') else str(secret_val)
                ...
                if 'result' in init_result:
                    ...
                    if tools_response.strip():
                        ...
                        if 'result' in tools_result and 'tools' in tools_result['result']:
                            ...
                            print(f"🔧 Discovered tools for {server_name}: {', '.join(tools)}")
                            return tools
