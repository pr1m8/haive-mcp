...
from .config import ServerConfig, ServerTemplate, InstallationMethod
...
class NPMInstaller(MCPInstaller):
    ...
    async def install(self, server_config: ServerConfig,
        ...
        package_json = {
            ...
            "version": "1.0.0",
            "dependencies": {
                package.split('@')[0]: server_config.version or "latest"
            }
        }
...
class BinaryInstaller(MCPInstaller):
    ...
    async def install(self, server_config: ServerConfig,
        ...
        if not download_url:
            return {
                "success": False,
                "error": "No download URL provided for binary"
            }
        ...
        try:
            ...
            return {
                "success": False,
                "error": f"Binary installation failed: {str(e)}"
            }
...
class CurlInstaller(MCPInstaller):
    ...
    async def install(self, server_config: ServerConfig,
        ...
        filename = server_config.variables.get(
            'filename',
            Path(urlparse(download_url).path).name or 'download'
        )
