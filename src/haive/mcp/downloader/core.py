...
class ServerStatus(BaseModel):
    ...
    last_success: Optional[datetime] = Field(None, description="Last success time")
    install_result: Optional[Dict[str, Any]] = Field(None, description="Install result")
...
class GeneralMCPDownloader:
    ...
    def __init__(self, config_file: Optional[str] = None, 
        ...
        logger.info(f"Initialized downloader with {len(self.config.templates)} templates "
    ...
    def _create_default_config(self) -> DownloaderConfig:
        ...
        """
        from .config import ServerTemplate, ServerConfig, DiscoveryConfig
        ...
        discovery = DiscoveryConfig(
            ...
            patterns={
                "npm": ["@modelcontextprotocol/server-*", "mcp-server-*", "*-mcp-server"],
    ...
    async def download_servers(self, server_names: Optional[List[str]] = None,
                             ...
                             max_concurrent: Optional[int] = None) -> DownloadResult:
        ...
        logger.info(f"Download complete: {successful}/{total} successful ({result.success_rate:.1f}%)")
        
        return result
    
    def _filter_servers(self, server_names: Optional[List[str]] = None,
                       categories: Optional[List[str]] = None,
                       tags: Optional[Set[str]] = None) -> List[ServerConfig]:
    ...
    async def _download_server(self, server: ServerConfig) -> Dict[str, Any]:
        ...
        for attempt in range(self.config.retry_attempts):
            ...
            try:
                result = await installer.install(server, template, self.config.install_dir)
    ...
    async def _verify_installation(self, server: ServerConfig,
                                  template: ServerTemplate,
                                  installer: MCPInstaller) -> bool:
    ...
    async def _save_server_config(self, successful_servers: List[Dict[str, Any]]) -> Path:
        ...
        if self.config.backup_enabled and config_path.exists():
            backup_path = self.config.backup_dir / f"mcp_servers_config_{int(time.time())}.json"
            import shutil
    ...
    async def auto_discover_and_download(self, limit: Optional[int] = None,
                                       auto_install: bool = True) -> DownloadResult:
        ...
        # Discover servers
        discovered_servers = await self.discovery.discover_all(
    ...
    async def check_server_health(self, server_names: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        if server_names:
            servers_to_check = [s for s in self.config.servers if s.name in server_names]
        else:
            # Check all installed servers
            servers_to_check = [
                s for s in self.config.servers
                if self.status_tracker.get(s.name, ServerStatus(name=s.name)).status == "installed"
            ]
        ...
        for server in servers_to_check:
            ...
            if installer:
                try:
                    is_healthy = await installer.verify(server, template, self.config.install_dir)
                    health_results[server.name] = "healthy" if is_healthy else "unhealthy"
                    
                    # Update status
                    if server.name in self.status_tracker:
                        self.status_tracker[server.name].health_status = health_results[server.name]
                        self.status_tracker[server.name].last_check = datetime.now()
                    
                except Exception as e:
                    health_results[server.name] = f"error: {str(e)}"
    ...
    def add_custom_server(self, server: ServerConfig) -> None:
        ...
        if any(s.name == server.name for s in self.config.servers):
            ...
            self.config.servers = [
                s if s.name != server.name else server
