...
class DiscoveredServer(BaseModel):
    ...
    author: Optional[str] = Field(None, description="Author/owner")
    version: Optional[str] = Field(None, description="Latest version")
    stars: Optional[int] = Field(None, description="Popularity metric")
...
class ServerDiscovery:
    ...
    async def discover_all(self, limit_per_source: Optional[int] = None) -> List[Dict[str, Any]]:
    ...
    async def discover_from_npm_registry(self, registry_url: str, 
                    ...
                    for obj in data.get('objects', []):
                        ...
                        if self._matches_npm_pattern(name):
                            ...
                            discovered.append({
                                ...
                                "tags": self._extract_tags(name, package.get('keywords', [])),
    ...
    async def discover_from_pypi(self, search_url: str, 
                               limit: int = 100) -> List[Dict[str, Any]]:
    ...
    async def discover_from_github(self, api_url: str,
                                 limit: int = 100) -> List[Dict[str, Any]]:
                        ...
                        if self._matches_github_pattern(name):
                            ...
                            discovered.append({
                                ...
                                "tags": self._extract_tags(
                                    name,
                                    repo.get('topics', []) + 
                                    (repo.get('language', '').lower().split() if repo.get('language') else [])
                                ),
                                "variables": {
    ...
    async def discover_from_url(self, url: str,
                              limit: int = 100) -> List[Dict[str, Any]]:
    ...
    def _extract_tags(self, name: str, keywords: List[str]) -> Set[str]:
        ...
        common_tags = {
            "mcp", "server", "ai", "llm", "tool", "api",
            "database", "file", "git", "web", "search"
        }
    ...
    def determine_template(self, server_data: Dict[str, Any]) -> str:
        ...
        if source == "npm":
            ...
            if server_data.get("repo_url"):
                ...
                if "python" in tags:
                    return "git_repo"
                elif "node" in tags or "npm" in tags:
    ...
    async def search_servers(self, query: str, 
        ...
        if "github" in sources:
            github_url = f"https://api.github.com/search/repositories?q={query}+mcp+server"
