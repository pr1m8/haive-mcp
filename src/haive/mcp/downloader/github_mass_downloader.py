...
from general_mcp_downloader import GeneralMCPDownloader, ServerConfig, ServerTemplate, InstallationMethod
...
class GitHubMCPDownloader:
    ...
    def load_all_servers(self):
        ...
        with open(self.all_servers_file, 'r') as f:
            return json.load(f)
    ...
    def categorize_servers(self, servers):
        ...
        for server in servers:
            ...
            if "@modelcontextprotocol" in name:
                category = "official"
            elif any(lang.lower() == "javascript" for lang in languages) or "npm" in name.lower():
                category = "npm"
    ...
    def create_server_config(self, server_data):
        ...
        # Clean up name
        name = name.replace("mcp-server-", "").replace("-mcp-server", "").replace("mcp-", "")
        if not name:
            name = "unknown"
        ...
        elif "npm" in metadata.get("name", "").lower() or any(lang.lower() == "javascript" for lang in languages):
    ...
    async def download_all_servers(self):
        ...
        for category, servers_list in categorized.items():
            ...
            downloader = GeneralMCPDownloader(
                install_dir=str(self.dirs[category])
            )
            ...
            if valid_servers:
                with Progress(
                    ...
                    task = progress.add_task(
                        f"Downloading {category} servers...",
    ...
    def create_master_config(self, categorized):
        ...
        for category, servers_list in categorized.items():
            config_file = self.dirs[category] / ".mcp" / "configs" / "mcp_servers_config.json"
        ...
        for category, dir_path in self.dirs.items():
            if dir_path.exists():
                ...
                subdirs = [d for d in dir_path.iterdir() if d.is_dir() and d.name != ".mcp"]
                if subdirs:
