...
class ServerTemplate(BaseModel):
    ...
    args_pattern: List[str] = Field(default_factory=list, description="Default arguments")
    ...
    health_check: Optional[str] = Field(None, description="Health check command")
    prerequisites: List[str] = Field(default_factory=list, description="System prerequisites")
...
class ServerConfig(BaseModel):
    ...
    version: Optional[str] = Field(None, description="Specific version constraint")
...
class DiscoveryConfig(BaseModel):
    ...
    """
    sources: List[str] = Field(default_factory=list, description="Discovery source URLs")
    patterns: Dict[str, List[str]] = Field(default_factory=dict, description="Naming patterns")
...
class DownloaderConfig(BaseModel):
    ...
    health_check_timeout: int = Field(default=30, gt=0, description="Health check timeout")
    backup_enabled: bool = Field(default=True, description="Enable backups")
    backup_dir: Path = Field(
        default_factory=lambda: Path.home() / ".mcp" / "backups",
        description="Backup directory"
    )
    templates: List[ServerTemplate] = Field(default_factory=list, description="Server templates")
    servers: List[ServerConfig] = Field(default_factory=list, description="Server configs")
    discovery: DiscoveryConfig = Field(default_factory=DiscoveryConfig, description="Discovery config")
