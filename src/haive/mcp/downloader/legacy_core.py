...
class MCPInstaller(ABC):
    ...
    @abstractmethod
    async def can_handle(self, server_config: ServerConfig, template: ServerTemplate) -> bool:
        """Check if this installer can handle the given configuration"""
        pass
    
    @abstractmethod
    async def install(self, server_config: ServerConfig, template: ServerTemplate, 
                     install_dir: Path) -> Dict[str, Any]:
        """Install the MCP server"""
        pass
    
    @abstractmethod
    async def verify(self, server_config: ServerConfig, template: ServerTemplate,
...
class NPMInstaller(MCPInstaller):
    ...
    async def _run_command(self, cmd: List[str], cwd: Optional[Path] = None, timeout: int = 300) -> Dict[str, Any]:
    ...
    async def install(self, server_config: ServerConfig, template: ServerTemplate, 
        ...
        for install_type in ["global", "local"]:
            try:
                ...
                result = await self._run_command(cmd, cwd=install_dir if install_type == "local" else None)
                ...
                if result["returncode"] == 0:
                    return {
                        ...
                        "command": f"npx {package}" if install_type == "global" else f"npm run {package}",
                        "success": True
                    }
...
class GitInstaller(MCPInstaller):
    ...
    async def install(self, server_config: ServerConfig, template: ServerTemplate,
        ...
        try:
            ...
            if result["returncode"] != 0:
                return {"success": False, "error": result.get("stderr", "Git clone failed")}
    ...
    async def install(self, server_config: ServerConfig, template: ServerTemplate,
        ...
        try:
            ...
            if result["returncode"] == 0:
                ...
                return {"success": False, "error": result.get("stderr", "Docker pull failed")}
        except Exception as e:
            return {"success": False, "error": str(e)}
...
class GeneralMCPDownloader:
    ...
    def __init__(self, config_file: Optional[str] = None, install_dir: Optional[str] = None):
    ...
    def load_config(self):
        ...
        try:
            ...
            for template_data in config.get('templates', []):
                ...
                if 'installation_method' in template_data and isinstance(template_data['installation_method'], str):
                    template_data['installation_method'] = InstallationMethod(template_data['installation_method'])
            ...
            logger.info(f"Loaded {len(self.templates)} templates and {len(self.servers)} servers")
    ...
    async def discover_servers_from_registry(self, registry_url: str) -> List[Dict[str, Any]]:
    ...
    def _parse_json_registry(self, data: Union[List, Dict]) -> List[Dict[str, Any]]:
    ...
    async def download_servers(self, server_names: Optional[List[str]] = None, 
        ...
        for server in servers_to_download:
            task = asyncio.create_task(self._download_server_with_semaphore(semaphore, server))
            tasks.append(task)
        ...
        for i, result in enumerate(results):
            ...
            if isinstance(result, Exception):
                ...
                failed.append({"server": server.name, "error": result.get("error", "Unknown error")})
    ...
    async def _download_server_with_semaphore(self, semaphore: asyncio.Semaphore, 
                                            server: ServerConfig) -> Dict[str, Any]:
    ...
    async def _download_server(self, server: ServerConfig) -> Dict[str, Any]:
        ...
        logger.info(f"Installing server: {server.name} using template: {server.template}")
        ...
        if not installer:
            return {"success": False, "error": f"No installer found for {template.installation_method}"}
        ...
        if result.get("success"):
            ...
            if await installer.verify(server, template, self.install_dir):
                ...
                logger.warning(f"Installation succeeded but verification failed: {server.name}")
    ...
    async def auto_discover_and_download(self, limit: Optional[int] = None) -> Dict[str, Any]:
...
async def main():
    ...
    parser.add_argument("--auto-discover", action="store_true", help="Auto-discover servers")
