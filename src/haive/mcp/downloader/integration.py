...
from .config import DownloaderConfig, ServerConfig
...
class MCPServerConnection(BaseModel):
    ...
    connection: Optional[Any] = Field(None, description="Connection object")
    tools: Dict[str, BaseTool] = Field(default_factory=dict, description="Discovered tools")
    resources: List[Dict[str, Any]] = Field(default_factory=list, description="Resources")
...
class MCPCapabilityExtractor:
    ...
    async def add_servers_from_config(self, servers: List[ServerConfig],
                                    config_dir: Path) -> Dict[str, bool]:
...
class MCPAgentIntegration:
    ...
    async def create_agent_with_mcp_servers(
        ...
        **agent_kwargs
        ...
        if missing_servers and install_if_missing:
            ...
            if result["failed"] > 0:
                logger.warning(f"Failed to install some servers: {result['failed_servers']}")
        ...
        **agent_kwargs
        ...
        if server_distribution == "shared":
            ...
            for i in range(num_agents):
                ...
                category_servers = [
                    s.name for s in self.downloader.servers
                    if s.enabled and s.template in self.downloader.templates
    ...
    def get_capability_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "prompts_by_server": {
                server: len(prm) for server, prm in prompts.items()
            }
        }
