...
from haive.mcp.manager import MCPManager, MCPServerStatus
...
async def demonstrate_procedural_mcp_addition():
    ...
    print(f"✅ Created MCPManager with health monitoring every {manager.health_check_interval}s")
    ...
    servers_to_add = [
        ...
        },
        {
            ...
            "config": MCPServerConfig(
                ...
                capabilities=["web_scraping", "screenshot", "pdf_generation", "automation"],
                category="web",
    ...
    for i, server_info in enumerate(servers_to_add, 1):
        ...
        try:
            ...
            if result.success:
                ...
                print(f"   🔧 Discovered {result.tools_count} tools: {', '.join(result.tools[:3])}{'...' if len(result.tools) > 3 else ''}")
    ...
    if all_tools:
        ...
        if len(all_tools) > 0:
            ...
            try:
                ...
                print(f"   Tool signature: {getattr(sample_tool, 'args_schema', 'No schema available')}")
            except Exception as e:
    ...
    print(f"\n❤️  Health Monitoring")
    print("-" * 18)
    print(f"Auto health checks: {'enabled' if manager.auto_health_check else 'disabled'}")
