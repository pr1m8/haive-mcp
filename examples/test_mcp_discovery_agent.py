...
class SimpleMCPDiscovery:
    ...
    def __init__(self):
        ...
        data_locations = [
            ...
            current_dir.parent.parent.parent.parent / "data" / "mcp_servers" / "ALL_MCP_SERVERS_COMPLETE.json",
            Path("/home/will/Projects/haive/backend/haive/data/mcp_servers/ALL_MCP_SERVERS_COMPLETE.json")
        ]
        ...
        if not self.data_path:
            print(f"❌ Could not find MCP data file. Searched in:")
            for path in data_locations:
                print(f"   - {path}")
    ...
    def load_data(self) -> bool:
        ...
        try:
            with open(self.data_path, 'r') as f:
                ...
                print(f"✅ Loaded {len(self.servers_data)} MCP servers from {self.data_path}")
                return True
    ...
    def search_tools(self, query: str, max_results: int = 5) -> List[Dict[str, Any]]:
        ...
        for server in self.servers_data:
            ...
            if (query_lower in name or 
                ...
                any(query_lower in tool for tool in tools)):
...
def create_mock_mcp_tool(server_info: Dict[str, Any]) -> Tool:
    ...
    def tool_function(input: str) -> str:
        ...
        if 'calculator' in server_name.lower():
            ...
            return f"Database query result: [simulated query '{input}' - returned 10 rows]"
        elif 'web' in server_name.lower() or 'scraper' in server_name.lower():
            return f"Web scraping result: [scraped content from '{input}' - 500 words extracted]"
        elif 'file' in server_name.lower():
            return f"File operation result: [performed operation on '{input}']"
        else:
            return f"[MCP Tool '{server_name}'] Processed: {input}"
    
    # Clean name for tool
    tool_name = server_name.replace('-', '_').replace(' ', '_').replace('@', '').replace('/', '_')
    ...
    try:
        ...
        for query in test_queries:
            ...
            try:
                ...
                print(f"🤖 Agent: I'll help you with that calculation.")
...
async def demo_database_agent():
    ...
    python_db_tools = [s for s in results if s.get('language', '').lower() in ['python', 'typescript', 'javascript']]
    ...
    try:
        ...
        for query in test_queries:
            ...
            try:
                ...
                print(f"🤖 Agent: I'll help you with that database query.")
