...
class MCPAugLLMConfig(MCPMixin, ToolRouteMixin, AugLLMConfig):
    ...
    async def setup(self) -> None:
        ...
        if hasattr(self, 'system_message') and self.system_message:
            self.system_message = self.enhance_system_prompt_with_mcp(self.system_message)
...
async def demonstrate_dataflow_server():
    ...
    server_path = Path(__file__).parent.parent / "src" / "haive" / "mcp" / "servers" / "dataflow_mcp_server.py"
    ...
    # Show dataflow tools
    mcp_tools = config.get_mcp_tools()
    logger.info(f"Dataflow tools available:")
    for tool in mcp_tools:
    ...
    mcp_config = MCPConfig(
        ...
        servers={
            ...
            "example": MCPServerConfig(
                ...
                command="python",
                args=[
                    str(Path(__file__).parent.parent / "src" / "haive" / "mcp" / "servers" / "example_server_fastmcp.py")
                ]
            )
        }
    )
