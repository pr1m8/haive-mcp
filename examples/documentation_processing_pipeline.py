...
class DocumentationProcessor:
    ...
    async def process_all_servers(self) -> Dict[str, Any]:
        ...
        async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=30)) as session:
            ...
            logger.info(f"🎯 Selected {len(selected_servers)} servers for documentation processing")
    ...
    def _select_servers_for_processing(self, servers: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        def score_server(server: Dict[str, Any]) -> float:
            ...
            high_value_categories = ['ai_ml', 'database', 'api_integration', 'search', 'cloud', 'version_control']
    ...
    async def _extract_all_documentation(self, servers: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    def _extract_github_info(self, repo_url: str) -> Optional[Dict[str, str]]:
    ...
    async def _get_repository_documentation(self, owner: str, repo: str) -> Optional[str]:
        ...
        try:
            async with self.session.get(url, headers=self._get_headers()) as response:
                if response.status == 200:
                    ...
                    logger.warning(f"Failed to get README for {owner}/{repo}: {response.status}")
                    return None
        except Exception as e:
        ...
        try:
            async with self.session.get(url, headers=self._get_headers()) as response:
                if response.status == 200:
                    ...
                    logger.warning(f"Failed to get repo info for {owner}/{repo}: {response.status}")
                    return None
        except Exception as e:
            logger.error(f"Error getting repo info for {owner}/{repo}: {e}")
            return None
    
    def _convert_to_standard_format(self, documented_servers: List[Dict[str, Any]]) -> Dict[str, Dict[str, Any]]:
    ...
    async def _save_documentation_database(self, formatted_docs: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if main_docs_file.exists():
            backup_file = self.servers_dir / f"all_mcp_documents_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        ...
        timestamped_file = self.servers_dir / f"processed_mcp_documents_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        ...
        results = {
            ...
            'summary': {
                ...
                'documentation_rate': (with_docs / len(formatted_docs) * 100) if formatted_docs else 0,
...
async def main():
    ...
    print(f"   Servers with documentation: {results['summary']['servers_with_documentation']}")
    ...
    print(f"\n📈 By Category:")
    for category, count in sorted(results['by_category'].items(), key=lambda x: x[1], reverse=True)[:10]:
        print(f"   {category}: {count}")
    
    print(f"\n📈 By Source:")
    for source, count in sorted(results['by_source'].items(), key=lambda x: x[1], reverse=True)[:5]:
    ...
    print(f"📄 Updated all_mcp_documents.json with {results['summary']['total_servers']} servers")
