...
class AnalysisResult(BaseModel):
    ...
    file_types: list[str] = Field(default_factory=list, description="Unique file types found")
...
async def example_custom_server():
    ...
    custom_server_path = Path(__file__).parent.parent / "src" / "haive" / "mcp" / "servers" / "dataflow_mcp_server.py"
    ...
    config = MCPAugLLMConfig(
        ...
        mcp_config=MCPConfig(
            enabled=True,
            servers={"dataflow": custom_server},
...
async def example_type_checking_benefits():
    ...
    mcp_config: Optional[MCPConfig] = config.mcp_config  # Properly typed
