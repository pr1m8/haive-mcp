...
async def download_by_pattern():
    ...
    db_servers = [name for name in all_servers if any(db in name.lower() for db in ['sql', 'db', 'mongo'])]
    ...
    if file_servers:
        ...
        console.print(f"File servers: {result['successful']}/{result['total']} successful")
    
    # Download database servers
    if db_servers:
        console.print("\n🗄️  Downloading database servers...")
        result = await downloader.download_servers(server_names=db_servers)
        console.print(f"Database servers: {result['successful']}/{result['total']} successful")
...
async def export_import_configs():
    ...
    console.print("2. Run: python scripts/manage_servers.py import mcp_servers_export.json")
