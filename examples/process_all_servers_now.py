...
async def process_all_servers():
    ...
    try:
        ...
        if processor.unprocessed_servers:
            print(f"⚡ Processing {len(processor.unprocessed_servers)} unprocessed servers...")
        ...
        print(f"⭐ Average quality score: {sum(m.quality_score for _, m in processor.processed_servers) / len(processor.processed_servers):.1f}")
        
        # Top 10 servers by quality
        print("\n🏆 Top 10 servers by quality:")
        top_servers = sorted(processor.processed_servers, key=lambda x: x[1].quality_score, reverse=True)[:10]
