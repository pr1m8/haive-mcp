...
class SimpleDiscoveryAgent:
        ...
        try:
            ...
            for server_name, server_doc in all_servers.items():
                ...
                if "filesystem" in server_name.lower():
                    analysis["categories"]["filesystem"] = analysis["categories"].get("filesystem", 0) + 1
                elif "github" in server_name.lower():
                    analysis["categories"]["github"] = analysis["categories"].get("github", 0) + 1
                elif "database" in server_name.lower() or "sql" in server_name.lower():
                    analysis["categories"]["database"] = analysis["categories"].get("database", 0) + 1
                elif "search" in server_name.lower():
                    analysis["categories"]["search"] = analysis["categories"].get("search", 0) + 1
                else:
                ...
                if "npm install" in content:
                    analysis["installation_methods"]["npm"] = analysis["installation_methods"].get("npm", 0) + 1
                if "pip install" in content:
                    analysis["installation_methods"]["pip"] = analysis["installation_methods"].get("pip", 0) + 1
                if "docker" in content:
                    analysis["installation_methods"]["docker"] = analysis["installation_methods"].get("docker", 0) + 1
    ...
    def _parse_capability_gaps(self, result) -> List[str]:
        ...
        if hasattr(result, 'content'):
            ...
            content = result.messages[-1].content if result.messages[-1].content else str(result.messages[-1])
        else:
            content = str(result)
