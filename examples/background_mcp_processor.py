...
class ServerQualityAssessor:
    ...
    async def assess_server(self, server_data: Dict[str, Any], 
                          session: aiohttp.ClientSession) -> ServerQualityMetrics:
        ...
        try:
            ...
            logger.debug(f"Assessed {name}: quality={quality_score:.1f}, "
    ...
    def _calculate_documentation_score(self, server_data: Dict[str, Any]) -> float:
        ...
        if doc:
            ...
            if any(keyword in doc_lower for keyword in ['install', 'setup', 'npm install', 'pip install']):
                score += 10.0
            
            # Usage examples
            if any(keyword in doc_lower for keyword in ['example', 'usage', '```', 'how to']):
                score += 10.0
            
            # Configuration documentation
            if any(keyword in doc_lower for keyword in ['config', 'configure', 'environment']):
                score += 5.0
            
            # API documentation
            if any(keyword in doc_lower for keyword in ['api', 'methods', 'functions', 'tools']):
                score += 5.0
    ...
    async def _calculate_popularity_score(self, server_data: Dict[str, Any], 
    ...
    def _calculate_maintenance_score(self, server_data: Dict[str, Any]) -> float:
        ...
        doc = server_data.get('documentation', '')
        if doc and any(keyword in doc.lower() for keyword in ['version', 'v1.', 'v2.', 'changelog']):
            score += 10.0
    ...
    def _calculate_completeness_score(self, server_data: Dict[str, Any]) -> float:
        ...
        # Required fields
        required_fields = [
            'name', 'repository_url', 'description', 'category',
        ...
        optional_fields = [
            'install_command', 'setup_instructions', 'npm_package',
            'transport_types', 'capabilities', 'dependencies'
        ]
        
        for field in optional_fields:
            total_fields += 1
            value = server_data.get('metadata', {}).get(field) if 'metadata' in server_data else server_data.get(field)
            if value:
                present_fields += 1
                score += 5.0
    ...
    def _categorize_server(self, server_data: Dict[str, Any]) -> Tuple[str, List[str]]:
        ...
        categories = {
            'ai_ml': {
                'keywords': ['ai', 'ml', 'machine learning', 'artificial intelligence', 'llm', 'gpt', 'model', 'embedding', 'vector', 'semantic'],
                'subcategories': ['language_models', 'embeddings', 'vector_search', 'semantic_search']
            },
            'database': {
                'keywords': ['database', 'sql', 'postgres', 'mysql', 'sqlite', 'mongo', 'redis', 'db', 'query', 'schema'],
                'subcategories': ['relational', 'nosql', 'cache', 'analytics']
            },
            'filesystem': {
                'keywords': ['file', 'filesystem', 'directory', 'folder', 'path', 'storage', 'disk'],
                'subcategories': ['file_operations', 'storage', 'backup']
            },
            'api_integration': {
                'keywords': ['api', 'rest', 'graphql', 'webhook', 'http', 'endpoint', 'service', 'integration'],
                'subcategories': ['rest_api', 'webhooks', 'microservices']
            },
            'version_control': {
                'keywords': ['git', 'github', 'gitlab', 'version', 'commit', 'repository', 'repo'],
                'subcategories': ['git_operations', 'collaboration', 'code_review']
            },
            'communication': {
                'keywords': ['slack', 'discord', 'teams', 'chat', 'message', 'notification', 'email'],
                'subcategories': ['messaging', 'notifications', 'collaboration']
            },
            'cloud': {
                'keywords': ['aws', 'azure', 'gcp', 'cloud', 'docker', 'kubernetes', 'container'],
            ...
            'security': {
                'keywords': ['auth', 'security', 'vault', 'secret', 'token', 'encryption', 'ssl'],
                'subcategories': ['authentication', 'encryption', 'secrets_management']
            },
            'productivity': {
                'keywords': ['notion', 'calendar', 'task', 'todo', 'project', 'management'],
                'subcategories': ['task_management', 'project_management', 'scheduling']
            },
            'finance': {
                'keywords': ['payment', 'stripe', 'paypal', 'finance', 'crypto', 'billing'],
            ...
            'monitoring': {
                'keywords': ['monitor', 'log', 'metric', 'alert', 'health', 'observability'],
    ...
    def _determine_install_method(self, server_data: Dict[str, Any]) -> str:
        ...
        if npm_package or 'npm install' in install_cmd:
            ...
            return 'pip'
        elif 'docker' in install_cmd.lower():
            return 'docker'
        elif 'git clone' in install_cmd.lower():
            return 'source'
    ...
    def _assess_setup_complexity(self, server_data: Dict[str, Any]) -> int:
        ...
        # Simple indicators
        if install_cmd and ('npm install' in install_cmd or 'pip install' in install_cmd):
            complexity -= 1
...
class DocumentationGenerator:
    ...
    async def generate_server_docs(self, server_data: Dict[str, Any], 
                                 metrics: ServerQualityMetrics) -> Path:
    ...
    def _generate_server_rst(self, server_data: Dict[str, Any], 
                           metrics: ServerQualityMetrics) -> str:
...
""".strip()
    ...
    def _generate_install_section(self, server_data: Dict[str, Any], 
                                metrics: ServerQualityMetrics) -> str:
...
"""
    ...
    def _extract_features_from_doc(self, doc: str) -> List[str]:
        ...
        for line in lines:
            ...
            if any(header in line.lower() for header in ['features', 'capabilities', 'what it does']):
...
"""
    ...
    def _generate_examples_rst(self, examples: List[Dict[str, str]], 
                             server_name: str) -> str:
...
"""
    ...
    def _generate_config_rst(self, server_data: Dict[str, Any], 
                           metrics: ServerQualityMetrics) -> str:
...
class CategoryOrganizer:
    ...
    def organize_servers(self, servers_with_metrics: List[Tuple[Dict[str, Any], ServerQualityMetrics]]) -> Dict[str, Any]:
        ...
        for server_data, metrics in servers_with_metrics:
            ...
            organized['by_install_method'][install_method].append((server_data, metrics))
        ...
        organized['by_popularity'].sort(key=lambda x: x[1].popularity_score, reverse=True)
        
        # Sort categories by quality
        for category in organized['by_category']:
            organized['by_category'][category].sort(key=lambda x: x[1].quality_score, reverse=True)
        
        # Calculate summary statistics
        organized['summary']['categories'] = list(organized['summary']['categories'])
        organized['summary']['install_methods'] = list(organized['summary']['install_methods'])
        organized['summary']['tags'] = list(organized['summary']['tags'])
        if servers_with_metrics:
            organized['summary']['average_quality'] = total_quality / len(servers_with_metrics)
        
        logger.info(f"Organized {len(servers_with_metrics)} servers into "
                   f"{len(organized['summary']['categories'])} categories")
        
        return organized
    
    def generate_category_summaries(self, organized_data: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
        ...
        for category, servers in organized_data['by_category'].items():
            ...
            summary = {
                ...
                'average_documentation': sum(m.documentation_score for m in metrics_list) / len(metrics_list),
                'average_popularity': sum(m.popularity_score for m in metrics_list) / len(metrics_list),
                'top_servers': [
                ...
                'complexity_distribution': self._get_complexity_distribution(metrics_list)
            }
    ...
    def _get_complexity_distribution(self, metrics_list: List[ServerQualityMetrics]) -> Dict[int, int]:
    ...
    def create_quality_rankings(self, organized_data: Dict[str, Any]) -> Dict[str, List]:
        ...
        for server_data, metrics in all_servers:
            if (metrics.quality_score > 70 and 
                'recently_updated' in metrics.tags and
                len(rankings['rising_stars']) < 10):
                rankings['rising_stars'].append((server_data, metrics))
        
        # Well documented servers
        well_documented = [
            (server_data, metrics) for server_data, metrics in all_servers
        ...
        easy_install = [
            (server_data, metrics) for server_data, metrics in all_servers
            if metrics.setup_complexity <= 2 and metrics.install_method in ['npm', 'pip']
        ]
...
class BackgroundMCPProcessor:
    ...
    def __init__(self, batch_size: int = 10, sleep_interval: int = 300, 
        ...
        logger.info(f"Initialized BackgroundMCPProcessor: batch_size={batch_size}, "
    ...
    async def run_continuous(self) -> None:
        ...
        try:
            while self.running:
                ...
                if not unprocessed:
                    logger.info("All servers processed, starting new cycle with updated data")
                ...
                logger.info(f"Processing batch {self.processing_stats['current_batch'] + 1}: "
                           f"{len(batch)} servers")
                ...
                self.processing_stats['last_update'] = datetime.now(timezone.utc).isoformat()
                ...
                if sleep_time > 0:
                    ...
                    logger.warning(f"Cycle took {cycle_time:.1f}s, "
                                  f"longer than interval {self.sleep_interval}s")
    ...
    async def _process_single_server(self, server_data: Dict[str, Any], 
                                   session: aiohttp.ClientSession) -> ServerQualityMetrics:
        ...
        try:
            ...
            doc_path = await self.doc_generator.generate_server_docs(server_data, metrics)
            
            # Save individual server data
            await self._save_server_data(server_data, metrics)
            
            logger.info(f"✅ Processed {server_name}: quality={metrics.quality_score:.1f}")
    ...
    async def _save_server_data(self, server_data: Dict[str, Any], 
                              metrics: ServerQualityMetrics) -> None:
        ...
        server_file = self.servers_dir / "processed" / f"{metrics.name.replace('/', '_')}.json"
    ...
    def _serialize_organized_data(self, data: Any) -> Any:
        ...
        if isinstance(data, dict):
            ...
            return {
                'server_data': server_data,
                'metrics': metrics.to_dict() if isinstance(metrics, ServerQualityMetrics) else metrics
            }
        elif isinstance(data, ServerQualityMetrics):
...
async def main():
    ...
    print(f"  Log file: logs/background_processor.log")
    print()
