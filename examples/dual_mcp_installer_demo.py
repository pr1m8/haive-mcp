...
async def demo_safe_pattern_installer():
    ...
    print(f"   Human approval: {'✅ Required' if filesystem_request.require_approval else '❌ Not required'}")
    ...
    print(f"   Human approval: {'✅ Required' if calculator_request.require_approval else '❌ Not required'}")
    ...
    if sample_config:
        print(f"   Claude Desktop Config Preview:")
        import json
        print(f"   {json.dumps(sample_config, indent=6)}")
    else:
        print(f"   No configuration found (server not installed yet)")
    
    print(f"\n✅ Safe Pattern Installer Demo Complete")
    print(f"   • Uses only trusted, predefined patterns")
    print(f"   • Human approval optional (configurable per server)")
    print(f"   • 90%+ success rate for standard MCP servers")
    print(f"   • Automatic .env and config file management")
...
async def demo_advanced_code_installer():
    ...
    print(f"   Human Approval: {'✅ Always Required' if status['human_approval_required'] else '❌ Optional'}")
    print(f"   LLM Agent: {'✅ Available' if status['llm_agent_available'] else '⚠️  Mock Mode'}")
    ...
    success, message, tools = await advanced_installer.install_server_advanced(standard_request)
    ...
    success, message, tools = await advanced_installer.install_server_advanced(python_request)
    print(f"   Success: {success}")
    ...
    success, message, tools = await advanced_installer.install_server_advanced(git_request)
    print(f"   Success: {success}")
    print(f"   Message: {message.split('Next Steps:')[0] if 'Next Steps:' in message else message}")
    print(f"   Tools Created: {len(tools)}")
    
    print(f"\n✅ Advanced Code Installer Demo Complete")
    print(f"   • Uses LLM agents for custom installation logic")
    print(f"   • Human approval ALWAYS required for generated code")
    print(f"   • Smart fallback to safe patterns when possible")
    print(f"   • Risk assessment and tolerance checking")
