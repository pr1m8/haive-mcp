...
class IntegratedMCPDiscoveryAgent(MCPAgent):
    ...
    def __init__(self, engine: Any, mcp_config: Optional[MCPConfig] = None, **kwargs):
    ...
    def _add_discovery_tools(self):
            ...
            for server_name, server_doc in all_servers.items():
                ...
                if query.lower() in searchable_text.lower():
                    matches.append({
                        ...
                        "npm_package": server_doc.get("metadata", {}).get("npm_package"),
                        "install_command": server_doc.get("metadata", {}).get("install_command"),
                        "capabilities": server_doc.get("metadata", {}).get("capabilities", [])
                    })
        ...
        def list_available_servers() -> str:
            ...
            summary = {
                ...
                "categories": {cat: len(servers) for cat, servers in categories.items()},
                "sample_servers": {
        ...
        def activate_mcp_server(server_name: str) -> str:
            ...
            return json.dumps({
                "status": "activated",
                "server_name": server_name,
                "available_tools": [f"{config['name']}_tool1", f"{config['name']}_tool2"],
...
When they want to use a specific capability, help them install and activate the appropriate server."""
    ...
    agent = IntegratedMCPDiscoveryAgent(
        engine=engine,
        name="mcp_discovery_assistant"
    )
    ...
    for query in test_queries:
        ...
        try:
            ...
            result = await agent.arun({"messages": [{"role": "user", "content": query}]})
            print(f"🤖 Agent: {result}")
        except Exception as e:
            # For demo purposes, show what the agent would do
            print(f"🤖 Agent: [Demo mode - would use discovery tools to answer]")
