...
class DynamicMCPManager:
    ...
    def __init__(self):
        ...
        data_path = Path(__file__).parent.parent / "data" / "mcp_servers" / "ALL_MCP_SERVERS_COMPLETE.json"
    ...
    def search_available_servers(self, capability: str, category: str = "") -> List[MCPServerInfo]:
        ...
        for server_data in self.all_servers:
            ...
            matches_capability = (
                capability_lower in name or 
                capability_lower in desc or
                any(capability_lower in tool for tool in tools)
            )
    ...
    async def assess_tool_needs(self, agent_request: str) -> ToolNeed:
        ...
        if any(word in request_lower for word in ['calculate', 'math', 'compute', 'add', 'multiply']):
            return ToolNeed(capability="calculator", category="math", priority="high")
        elif any(word in request_lower for word in ['file', 'read', 'write', 'directory', 'folder']):
            return ToolNeed(capability="filesystem", category="file", priority="high")
        elif any(word in request_lower for word in ['web', 'scrape', 'url', 'website', 'html']):
            return ToolNeed(capability="web_scraping", category="web", priority="medium")
        elif any(word in request_lower for word in ['database', 'sql', 'query', 'table']):
    ...
    def find_missing_tools(self, needed: ToolNeed) -> List[MCPServerInfo]:
        ...
        has_capability = any(needed.capability in tool.lower() for tool in current_tools)
        ...
        installable = [s for s in candidates if s.install_command and not s.is_installed]
        
        print(f"🔍 Found {len(installable)} installable servers for {needed.capability}")
        ...
        def execute_mcp_tool(query: str) -> str:
            ...
            try:
                ...
                request = {
                    ...
                    "params": {
                        "name": tool_name,
                        "arguments": {"query": query} if tool_name in ["calculate", "search"] else {"path": query}
                    }
                }
    ...
    async def auto_discover_and_install_tools(self, server_info: MCPServerInfo) -> List[StructuredTool]:
            ...
            if response.strip():
                ...
                if 'result' in result and 'tools' in result['result']:
                    ...
                    for tool_info in available_tools:
                        ...
                        if tool_name:
                            dynamic_tool = self.create_dynamic_tool(server_info.name, tool_name)
