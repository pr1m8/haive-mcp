...
class MCPDiscoveryWorkflow:
    ...
    async def step1_search_for_tools(self, search_query: str) -> List[Dict[str, Any]]:
        ...
        search_results = [
            ...
            },
            {
                ...
                "capabilities": ["expression_parsing", "advanced_math", "unit_conversion"]
            },
            {
    ...
    async def step4_create_agent_with_tool(self, mcp_config: MCPConfig) -> MCPAgent:
        ...
        try:
            ...
            print(f"⚠️  Note: MCP server initialization requires the actual npm package")
            print(f"   Error: {e}")
            print(f"   In production, the MCP server would be automatically started")
    ...
    async def step5_demo_usage(self, agent: MCPAgent):
        ...
        for query in test_queries:
            ...
            try:
                ...
                print(f"🤖 Agent: [Simulated response - actual MCP server not running]")
                if "15 * 23" in query:
...
async def main():
    ...
    selected_tool = await workflow.step2_show_installation(tool_index=1)  # Select mathjs-mcp-server
