...
async def run_dataflow_mcp_example():
    ...
    server_script = Path(__file__).parent.parent / "src" / "haive" / "mcp" / "servers" / "dataflow_mcp_server.py"
    ...
    async with stdio_client(server_params) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            logger.info(f"Registry entities by type:")
            ...
            logger.info(f"\nRegistry statistics:")
            logger.info(f"  - Total entities: {stats_resource.get('total_entities', 0)}")
...
async def demonstrate_mcp_integration():
    ...
    try:
        ...
        config = await create_mcp_enabled_aug_config(
            ...
            mcp_servers={
                "dataflow": MCPServerConfig(
                    ...
                    command="python",
                    args=[
                        str(Path(__file__).parent.parent / "src" / "haive" / "mcp" / "servers" / "dataflow_mcp_server.py")
                    ]
                )
            },
        ...
        logger.info(f"MCP tools discovered: {len([t for t in config.tools if t.startswith('dataflow_')])}")
