...
class MCPSourceHarvester:
    ...
    def _categorize_server(self, name: str, description: str = "") -> str:
        ...
        categories = {
            ...
            'database': ['database', 'sql', 'mongo', 'redis', 'postgres', 'mysql', 'sqlite'],
...
class GitHubRepoHarvester(MCPSourceHarvester):
    ...
    def __init__(self, session: aiohttp.ClientSession, repo_url: str, github_token: Optional[str] = None):
    ...
    async def _get_repo_info(self) -> Dict[str, Any]:
        ...
        async with self.session.get(url, headers=self._get_headers()) as response:
            if response.status == 200:
                ...
                logger.warning(f"Failed to get repo info for {self.owner}/{self.repo}: {response.status}")
                return {}
    ...
    async def _parse_awesome_list(self, readme_content: str, repo_info: Dict[str, Any]) -> List[MCPServerRecord]:
        ...
        for name, url in matches:
            if 'mcp' in name.lower() or 'mcp' in url.lower():
                ...
                description = self._extract_description_from_context(readme_content, url)
    ...
    async def _parse_official_repo(self, repo_info: Dict[str, Any]) -> List[MCPServerRecord]:
        ...
        async with self.session.get(url, headers=self._get_headers()) as response:
            if response.status == 200:
                ...
                for item in contents:
                    if item['type'] == 'dir' or (item['name'].endswith('.js') or item['name'].endswith('.ts')):
                        server_name = item['name'].replace('.js', '').replace('.ts', '')
                        
                        if 'server' in server_name.lower() or 'mcp' in server_name.lower():
                            server = MCPServerRecord(
    ...
    async def _parse_generic_repo(self, readme_content: str, repo_info: Dict[str, Any]) -> List[MCPServerRecord]:
        ...
        server = MCPServerRecord(
            ...
            category=self._categorize_server(self.repo, repo_info.get('description', '')),
...
class GitHubTopicHarvester(MCPSourceHarvester):
    ...
    def __init__(self, session: aiohttp.ClientSession, topic: str, github_token: Optional[str] = None):
                    ...
                    if response.status == 200:
                        ...
                        for repo in items:
                            ...
                            server = MCPServerRecord(
                                ...
                                category=self._categorize_server(repo['name'], repo.get('description', '')),
                                tags=[self.topic],
...
class RegistryWebHarvester(MCPSourceHarvester):
    ...
    def __init__(self, session: aiohttp.ClientSession, registry_name: str, base_url: str):
    ...
    async def harvest(self) -> List[MCPServerRecord]:
        ...
        try:
            ...
            elif self.registry_name.lower() == 'smithery':
                return await self._harvest_smithery()
            elif self.registry_name.lower() == 'mcpregistry':
    ...
    def _parse_pulsemcp_data(self, data: Any) -> List[MCPServerRecord]:
        ...
        for item in items:
            if isinstance(item, dict):
                github_info = self._extract_github_info(item.get('repository', item.get('url', '')))
                ...
                server = MCPServerRecord(
                    ...
                    category=self._categorize_server(item.get('name', ''), item.get('description', '')),
...
class ProductionMCPHarvester:
    ...
    async def harvest_all(self) -> Dict[str, Any]:
        ...
        async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=30)) as session:
            ...
            for source in sources:
                try:
                    ...
                    logger.error(f"❌ Failed to harvest {source.__class__.__name__}: {e}")
        ...
        logger.info(f"✅ Harvest complete! {len(unique_servers)} unique servers from {self.sources_processed}/{self.total_sources} sources")
    ...
    def _define_sources(self) -> List[MCPSourceHarvester]:
        ...
        for repo_url in github_repos:
            sources.append(GitHubRepoHarvester(self.session, repo_url, self.github_token))
    ...
    async def _harvest_source(self, harvester: MCPSourceHarvester):
        ...
        try:
            ...
            logger.info(f"📊 Progress: {self.sources_processed + 1}/{self.total_sources} sources")
        except Exception as e:
            logger.error(f"❌ Source {harvester.source_name} failed: {e}")
    
    def _deduplicate_servers(self, servers: List[MCPServerRecord]) -> List[MCPServerRecord]:
    ...
    def _generate_analytics(self, servers: List[MCPServerRecord]) -> Dict[str, Any]:
        ...
        analytics['recent_updates'] = [
            {'name': s.name, 'last_updated': s.last_updated, 'repository_url': s.repository_url}
...
async def main():
    ...
    print(f"   Success rate: {results['sources_processed']}/{results.get('total_sources', 'unknown')}")
