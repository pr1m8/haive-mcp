...
async def main():
    ...
    with open(production_db, 'r') as f:
        data = json.load(f)
    ...
    for i, server in enumerate(all_servers):
        try:
            ...
            if (i + 1) % 100 == 0:
                print(f"  Processed {i + 1}/{total} servers ({(i+1)/total*100:.1f}%)")
                
        except Exception as e:
            failed.append(server)
            print(f"  ❌ Failed: {server.get('name', 'Unknown')} - {str(e)}")
    ...
    await processor._generate_batch_documentation(processed, organized, summaries, rankings)
    ...
    for category, servers in sorted(organized['by_category'].items(), 
