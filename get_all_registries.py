...
async def fetch_pulsemcp_servers(session: aiohttp.ClientSession) -> List[Dict]:
    ...
    try:
        ...
        if not servers:
            ...
            async with session.get("https://www.pulsemcp.com/servers", timeout=30) as resp:
                if resp.status == 200:
...
async def fetch_github_topic_servers(session: aiohttp.ClientSession, topic: str) -> List[Dict]:
...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        # Process results
        sources = ['PulseMCP', 'Smithery', 'MCP Registry', 'GitHub:mcp-server', 
                  'GitHub:model-context-protocol', 'TensorBlock']
    ...
    output = {
        "metadata": {
            ...
            "sources": {
                "PulseMCP": len([s for s in all_servers.values() if 'pulsemcp' in s.get('source', '').lower()]),
                "Smithery": len([s for s in all_servers.values() if 'smithery' in s.get('source', '').lower()]),
                "MCP Registry": len([s for s in all_servers.values() if 'mcpregistry' in s.get('source', '').lower()]),
                "GitHub Topics": len([s for s in all_servers.values() if 'github-topic' in s.get('source', '').lower()]),
                "TensorBlock": len([s for s in all_servers.values() if 'TensorBlock' in s.get('source', '')])
            }
        },
