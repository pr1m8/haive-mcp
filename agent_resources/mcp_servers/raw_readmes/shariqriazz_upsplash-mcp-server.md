...
_(Note: Replace `@shariqriazz` with the correct username/namespace if needed)_

**Git Repository:** [https://github.com/shariqriazz/upsplash-mcp-server](https://github.com/shariqriazz/upsplash-mcp-server) _(Please update if this is not the correct URL)_
    ...
    ```json
    {
      ...
      "properties": {
        ...
        "resolution": {
          ...
          "description": "Desired resolution (default: raw).",
          "default": "raw"
        },
        "filename": {
          "type": "string",
          "description": "Optional filename (e.g., my-image.jpg). Defaults to sanitized description or {photo_id}.jpg."
        }
      },
      "required": ["photo_id"]
    }
...
_(Note: Schemas are illustrative. The server exposes the exact schemas via MCP.)_
...
_Note: This requires the package to be published on npm under the specified name._
    ...
    ```json
    {
      "mcpServers": {
        "unsplash-mcp-server": {
          ...
          "disabled": false,
          "alwaysAllow": ["search_photos", "download_photo"],
...
_(Note: Assumes standard `package.json` scripts like `watch`, `lint`, `format`. Update if your scripts differ.)_
