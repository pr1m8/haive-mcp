...
class MCPSetupManager:
    ...
    def _check_python_version(self) -> bool:
        ...
        try:
            ...
            if version.major < 3 or (version.major == 3 and version.minor < 12):
                self.errors.append(f"Python 3.12+ required, found {version.major}.{version.minor}")
                return False
    ...
    async def _install_dependencies(self) -> bool:
        ...
        dependencies = [
            ...
            ("npm", "sudo apt-get install -y npm" if sys.platform == "linux" else "brew install npm"),
            ...
            try:
                ...
                if install_cmd:
                    ...
                    if sys.platform == "win32":
                        ...
                        self.warnings.append(f"Manual installation required for {dep} on Windows")
                    else:
                        subprocess.run(install_cmd, shell=True, check=True)
                        logger.info(f"✓ {dep} installed successfully")
                else:
                    self.warnings.append(f"{dep} not found, may need manual installation")
        ...
        for server in servers:
            try:
                ...
                if "env_required" in server:
                    missing_env = [
                        var for var in server["env_required"] 
...
'''
                    ...
                    if result.returncode == 0:
                        ...
                        self.warnings.append(f"Failed to register {server_name}: {result.stderr}")
