...
Developer-friendly & type-safe Typescript SDK specifically catered to leverage _docusign-test_ API.
...
Some methods specify known errors which can be thrown. All the known errors are enumerated in the `models/errors/errors.ts` module. The known errors for a method are documented under the _Errors_ tables in SDK docs. For example, the `list` method may throw the following errors:
