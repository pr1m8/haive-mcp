...
The server will start and listen for MCP connections via standard input/output (stdio). You should see a message like `Calculator MCP Server connected via stdio and ready.` printed to your terminal's _standard error_. Keep this terminal window open while using the server with Claude Desktop.
