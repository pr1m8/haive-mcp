...
def config(server_name: str, key: Optional[str], value: Optional[str], 
           env: tuple, arg: tuple):
...
def health(test_all: bool):
    ...
    console.print(Panel.fit(
        "[bold cyan]MCP Server Health Check[/bold cyan]",
        title="Health Status"
    ))
...
def import_(config_file: str, merge: bool):
    ...
    try:
        ...
        if result['success']:
            console.print(f"[green]Successfully imported {result['imported']} servers[/green]")
            if result.get('skipped'):
                console.print(f"[yellow]Skipped {result['skipped']} existing servers[/yellow]")
        else:
