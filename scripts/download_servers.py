...
from src.haive.mcp.downloader.legacy_core import GeneralMCPDownloader, ServerConfig
...
@cli.command()
@click.option('--config', '-c', help='Configuration file path', default='configs/default_config.yaml')
...
@click.option('--all', 'download_all', is_flag=True, help='Download all configured servers')
...
def download(config: str, servers: tuple, category: tuple, download_all: bool, 
             max_concurrent: int, output_dir: Optional[str]):
...
def discover(source: Optional[str], limit: Optional[int], output: Optional[str]):
...
@click.option('--force', '-f', is_flag=True, help='Force update even if already installed')
