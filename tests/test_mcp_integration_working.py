...
async def test_mcp_with_langchain():
    ...
    server_config = {
        "example-server": {
            "transport": "stdio",
            "command": "python",
            "args": [str(Path(__file__).parent / "src/haive/mcp/servers/example_server_fastmcp.py")]
        }
    }
        ...
        if tools:
            ...
            if list_dir_tool:
                ...
                try:
                    ...
                    logger.info(f"Found {len(result) if isinstance(result, list) else 'unknown'} Python files")
