...
async def test_mcp_integration():
    ...
    try:
        ...
        for server in mcp_servers:
            if isinstance(server, dict):
                logger.info(f"  - {server.get('name', 'Unknown')}: {server.get('description', 'No description')}")
            else:
    ...
    try:
        ...
        if success:
            ...
            logger.warning("⚠ MCP client initialization returned False (this may be expected if no real servers are configured)")
    except Exception as e:
