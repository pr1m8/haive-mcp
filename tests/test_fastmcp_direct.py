...
async def test_fastmcp_tools():
    ...
    for tool_name, tool_func in example_mcp._tools.items():
        logger.info(f"  - {tool_name}: {tool_func.__doc__.strip() if tool_func.__doc__ else 'No description'}")
    ...
    try:
        ...
        result = await example_mcp._tools["write_file"](test_file, "Hello from MCP server!")
...
async def test_fastmcp_prompts():
    ...
    for prompt_name, prompt_func in example_mcp._prompts.items():
        logger.info(f"  - {prompt_name}: {prompt_func.__doc__.strip() if prompt_func.__doc__ else 'No description'}")
