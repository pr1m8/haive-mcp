...
class TestMCPWithMockServer:
    ...
    async def test_successful_mcp_initialization(self, test_engine, mock_mcp_config):
        ...
        agent = MockMCPAgent(engine=test_engine, mcp_config=mock_mcp_config)

        # Mock the MCP dependencies and client
        with (
            patch("haive.mcp.mixins.mcp_mixin.MCP_AVAILABLE", True),
            patch(
                "haive.mcp.mixins.mcp_mixin.MultiServerMCPClient"
    ...
    async def test_dynamic_server_refresh(self, test_engine):
        ...
        agent = MockMCPAgent(engine=test_engine, mcp_config=initial_config)
        ...
        with (
            ...
            with patch.object(agent, "_test_server_connection", return_value=True):
                ...
                new_tools = [MockTool("read_file"), MockTool("write_file")]
    ...
    async def test_server_failure_handling(self, test_engine):
        ...
        config = MCPConfig(
            ...
            servers={
                "good_server": MCPServerConfig(
                    name="good_server", transport="stdio", command="good_cmd"
                ),
                "bad_server": MCPServerConfig(
                    name="bad_server", transport="stdio", command="bad_cmd"
                ),
            },
        )

        agent = MockMCPAgent(engine=test_engine, mcp_config=config)
        ...
        with (
            ...
            with patch.object(
                agent, "_test_server_connection", side_effect=mock_test_connection
            ):
                result = await agent.initialize_mcp()
...
class TestMCPIntegrationScenarios:
    ...
    async def test_research_workflow_with_mcp(self, test_engine):
        ...
        with (
            ...
            with patch.object(agent, "_test_server_connection", return_value=True):
                ...
                file_result = await agent.call_mcp_tool(
                    "read_file", {"path": "research.txt"}
                )
                assert "research.txt" in file_result

                web_result = await agent.call_mcp_tool(
    ...
    async def test_collaborative_agents_workflow(self, test_engine):
        ...
        reader_agent = MockMCPAgent(
            engine=test_engine, mcp_config=shared_config, name="reader"
        )
        writer_agent = MockMCPAgent(
            engine=test_engine, mcp_config=shared_config, name="writer"
        )
        ...
        with (
            ...
            with (
                patch.object(
                    reader_agent, "_test_server_connection", return_value=True
                ),
                patch.object(
                    writer_agent, "_test_server_connection", return_value=True
                ),
            ):
                # Setup both agents
                ...
                read_result = await reader_agent.call_mcp_tool(
                    "query", {"sql": "SELECT * FROM data"}
                )
                write_result = await writer_agent.call_mcp_tool(
