...
from src.haive.mcp.downloader.legacy_core import GeneralMCPDownloader, ServerConfig, ServerTemplate, InstallationMethod
...
class TestGeneralMCPDownloader:
    ...
    def test_default_templates_loaded(self, downloader):
        ...
        expected_templates = ['npm_official', 'npm_community', 'git_repo', 'docker_image']
    ...
    async def test_download_nonexistent_server(self, downloader):
        ...
        result = await downloader.download_servers(
            server_names=["nonexistent-server-12345"],
            max_concurrent=1
        )
    ...
    def test_installer_availability(self, downloader):
        ...
        installer_types = [type(installer).__name__ for installer in downloader.installers]
        
        expected_installers = ["NPMInstaller", "PipInstaller", "GitInstaller", "DockerInstaller"]
    ...
    async def test_concurrent_downloads(self, downloader):
        ...
        if len(downloader.servers) > 1:
            ...
            other_servers = [s.name for s in downloader.servers if s.name != "filesystem" and s.enabled]
            if other_servers:
                servers_to_test.append(other_servers[0])
        
        result = await downloader.download_servers(
            server_names=servers_to_test,
            max_concurrent=3
        )
...
class TestDownloaderEdgeCases:
    ...
    async def test_missing_template(self, tmp_path):
        ...
        failed_server = result["failed_servers"][0]
        assert "Template" in failed_server["error"] or "not found" in failed_server["error"]
