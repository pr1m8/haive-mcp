...
from src.haive.mcp.downloader import InstallationMethod, ServerConfig, ServerTemplate
from src.haive.mcp.downloader.installers import (
    DockerInstaller,
    GitInstaller,
    NPMInstaller,
    PipInstaller,
...
class TestInstallerTypes:
        ...
        for test_case in test_cases:
            ...
            try:
                ...
                can_handle = await installer.can_handle(
                    test_case["config"], test_case["template"]
                )
                assert can_handle, f"NPM installer should handle {test_case['name']}"

                # Install
                result = await installer.install(
                    test_case["config"], test_case["template"], test_dir
                )
                ...
                console.print(
                    f"✓ Success: {result['success']}, Verified: {result.get('verified', False)}"
                )
    ...
    def _display_results(self, title: str, results: list):
        ...
        for result in results:
            ...
            verified_mark = (
                "✓" if result.get("verified") else "✗" if "verified" in result else "-"
            )
            verified_style = (
                "green"
                if result.get("verified")
                else "red" if "verified" in result else "dim"
            )
