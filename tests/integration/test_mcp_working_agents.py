...
class TestMCPWithWorkingAgents:
    ...
    def test_mcp_simple_agent_creation(self, test_engine, test_mcp_config):
        ...
        agent = MCPSimpleAgent(
            engine=test_engine, mcp_config=test_mcp_config, name="mcp_simple_agent"
        )
    ...
    async def test_mcp_simple_agent_setup(self, test_engine, test_mcp_config):
        ...
        agent = MCPSimpleAgent(engine=test_engine, mcp_config=test_mcp_config)
    ...
    def test_mcp_react_agent_creation(self, test_engine, test_mcp_config):
        ...
        agent = MCPReactAgent(
            engine=test_engine, mcp_config=test_mcp_config, name="mcp_react_agent"
        )
    ...
    async def test_mcp_react_agent_with_tools(self, test_engine):
        ...
        agent = MCPReactAgent(
            engine=test_engine, mcp_config=config, force_tool_use=True
        )
        ...
        assert (
            len(status["failed_servers"]) > 0 or len(status["connected_servers"]) == 0
        )
    ...
    async def test_dynamic_mcp_with_simple_agent(self, test_engine):
        ...
        agent = MCPSimpleAgent(engine=test_engine, mcp_config=MCPConfig(enabled=False))
        ...
        agent.mcp_config = MCPConfig(
            ...
            servers={
                "dynamic": MCPServerConfig(
                    name="dynamic", transport="stdio", command="echo", args=["Dynamic"]
                )
            },
        )
            ...
            mcp_config=MCPConfig(
                ...
                servers={
                    "web_ops": MCPServerConfig(
                        name="web_ops", transport="sse", url="http://localhost:8080/mcp"
                    )
                },
            ),
            name="web_agent",
        ...
        config = MCPConfig(
            ...
            servers={
                "lazy": MCPServerConfig(
                    name="lazy", transport="stdio", command="echo", args=["Lazy"]
                )
            },
        )

        agent = MCPReactAgent(engine=test_engine, mcp_config=config)
    ...
    async def test_mcp_server_discovery(self, test_engine):
        ...
        config = MCPConfig(enabled=True, auto_discover=True, servers={})

        agent = MCPSimpleAgent(engine=test_engine, mcp_config=config)
    ...
    async def test_real_filesystem_server_if_available(self, test_engine):
        ...
        agent = MCPSimpleAgent(engine=test_engine, mcp_config=config, name="fs_agent")
...
class TestMCPMultiAgentScenarios:
    ...
    async def test_mcp_agent_collaboration(self, test_engine):
        ...
        agent1 = MCPSimpleAgent(
            engine=test_engine, mcp_config=shared_config, name="agent1"
        )

        agent2 = MCPReactAgent(
            engine=test_engine, mcp_config=shared_config, name="agent2"
        )
